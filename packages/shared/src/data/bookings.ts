...
import { prixColisDepuisTarifTrajet } from "../pricing";
import type { ActionResult, CreateBookingInput } from "../types";
...
export async function createBookingRecord(
  ...
  if (input.bookingType === "PASSENGER") {
    ...
    totalPrice = prixColisDepuisTarifTrajet(
      Number(trip.parcel_base_price),
      Number(trip.parcel_price_per_kg),
      weight,
    );
    otp = generateOtp(6);
  ...
  if (otp) {
    await client.from("booking_secrets").insert({
...
export async function rejectBookingRecord(
  client: SupabaseClient,
  bookingId: string,
): Promise<ActionResult> {
  ...
  if (booking?.booking_type === "PASSENGER" && booking.trips) {
    ...
    if (trip) {
      await client
        .from("trips")
        .update({
          available_seats: Number(trip.available_seats) + booking.seats_booked,
        })
...
export async function markBookingPickedUp(
  client: SupabaseClient,
  bookingId: string,
  proofUrl?: string,
): Promise<ActionResult> {
...
export async function verifyDeliveryOtpCode(
  client: SupabaseClient,
  bookingId: string,
  code: string,
): Promise<ActionResult> {
  const { data, error } = await client.rpc("verify_delivery_otp", {
