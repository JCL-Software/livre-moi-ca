...
const PawPrintIcon = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
  (
    ...
    const stop = useCallback(() => {
      animateFn(".paw-inner", { opacity: 1, scale: 1, y: 0 }, { duration: 0.3 });
    }, [animateFn]);
    ...
    useEffect(() => {
      ...
      if (animate) void start();
      else stop();
    }, [animate, start, stop]);

    const parentControlled = animate !== undefined;

    return (
      <motion.svg
        ref={scope}
        onHoverStart={parentControlled ? undefined : start}
        onHoverEnd={parentControlled ? undefined : stop}
